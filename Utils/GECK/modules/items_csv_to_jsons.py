...
def items_csv_to_jsons():
    ...
    delimiter = input("Bitte geben Sie das Trennzeichen der"\
                      "CSV-Datei an (Standardmäßig ein Komma): ") or ","
    json_folder = "generated_items"
    os.makedirs(json_folder, exist_ok=True)
    json_folder = "generated_items"
    os.makedirs(json_folder, exist_ok=True)  # Unterordner erstellen, falls er nicht existiert
    ...
    with open(csv_file, 'r', encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            with open(json_path, 'w', encoding="utf-8") as json_file:
                json.dump({"$schema": "../../.github/workflows/itemschema.json", replace_umlauts(row['Name']): json_data}, json_file, indent=4, ensure_ascii=True)


    print(f"Konvertierung abgeschlossen! Die JSON-Dateien wurden im Ordner '{json_folder}' erstellt.") 
